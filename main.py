...
def help_box(event=None):
    messagebox.showinfo("Help", "For refer to book:\n Tkinter Gui Application\n Development Hotshot",icon="question")
...
def open_file(event=None):
    ...
    if filename == "": # If no file chosen.
        ...
        root.title(os.path.basename(filename) + " - nksworld") # Returning the basename of 'file'
        textPad.delete(1.0,END)         
        fh = open(filename,"r")        
...
def save_as(event=None):
    try:
        ...
        fh.write(textoutput)              
        fh.close()     			  
        root.title(os.path.basename(f) + " - nksworld") # Setting the title of the root widget.
    except:
        pass
#####################################################
...
icons = [
    'new_file','open_file','save','cut','copy','paste','undo','redo','on_find','about'
]
